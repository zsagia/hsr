...
(function (root, factory) {
  ...
  var Collapse = function (element, options) {
    ...
    var getOuterHeight = function (el) {
      ...
      return el.clientHeight + parseInt(btp) + parseInt(mtp) + parseInt(mbp) + parseInt(mte) + parseInt(mbe); //we need an accurate margin
                                                                                                              // value
    };
    ...
    this.open = function () {
      ...
      if (this.accordion!==null) {
        var active = this.accordion.querySelectorAll('.collapse.in'),
          ...
          i = 0;
        for (i; i<al; i++) {
          if (active[i]!==this.collapse) {
            this._close(active[i]);
          }
        }
      }
    };
  ...
  var Tab = function (element, options) {
    ...
    this.handle = function (e) {
      ...
      var nextContent = document.getElementById(next.getAttribute('href').replace('#', '')); //this is the actual object, the next tab
                                                                                             // content to activate
